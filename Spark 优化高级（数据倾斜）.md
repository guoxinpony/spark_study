...
- 原本能够正常执行的Spark作业，某天突然报出**OOM**（内存溢出）异常，观察异常栈，是业务代码造成的。
- **某个key对应的数据量特别大**的话，就会发生数据倾斜。整个Spark作业的运行进度是由运行时间最长的task决定的。
...
1. **思路**：对于少数几个数据量特别多的key，如果对作业的执行和计算结果**不是特别重要**的话，**那么就直接过滤掉那少数几个**key。
...
3. **优点：**对所有类型的 Join 数据倾斜基本都能处理，不受倾斜 Key 数量的限制；大幅缩短长尾任务，显著提升作业执行效率，实现负载均衡，所有机器都分到了差不多的任务量
