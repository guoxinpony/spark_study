...
2. **分区写入：** 每一个 Task 会根据 Reducer 的数量，直接**为每个分区开启一个临时文件**并写入数据
...
1.  **下游分区数 (reduce task 数量) 较少：**默认需要小于或等于 200；如果分区太多，同时开启过多的临时文件会导致内存溢出。z
...
| **比较维度** | **标准 SortShuffle **                                        | **BypassShuffle**                                            |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
